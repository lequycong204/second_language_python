class Village:
    ...
    def get_max_change(self):
        ...
        """
        if len(self.student) < 2:
            return (None, 0)
        years = sorted(self.student.keys())
        max_increase = 0
        max_year = None
        for i in range(1, len(years)):
            prev_year = years[i - 1]
            current_year = years[i]
            increase = self.student[current_year] - self.student[prev_year]
            if increase > max_increase:
                max_increase = increase
                max_year = current_year
        return (max_year, max_increase)