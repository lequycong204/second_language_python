...
class TFIDF:
    ...
    def loadCorpus(self,corpusPath):
        ...
        for fname in os.listdir(corpusPath):
            path = os.path.join(corpusPath, fname)
            if os.path.isfile(path):
                self.data[fname] = Doc(path)
    ...
    def tf(self,w,d):
        ...
        tfm = max(d.wordCount.values(), default=1)
        return tfw / tfm if tfm != 0 else 0
    ...
    def idf(self, w):
        ...
        '''
        N = len(self.data)
        m = sum(1 for d in self.data.values() if w in d.wordCount)
        return math.log(N / (1 + m)) if m >= 0 else 0
    ...
    def getKTopicWordFromCopus(self, k):
        ...
        for doc in self.data.values():
            for word in doc.wordCount:
                ...
                if word not in tfidf_scores or tfidf_scores[word] < score:
                    tfidf_scores[word] = score
        top_k = sorted(tfidf_scores.items(), key=lambda x: -x[1])[:k]
        return sorted([w for w, _ in top_k])
